...
- Sign blocks: have different styles, and compared to vanilla signs, signs in this mod can flexibly adjust parameters such as color, size and position. Sign presets can be managed via the `/mishanguc:signpreset` command.
- Handrail blocks: have various textures, outlook and connecting method, and can be placed on stairs.
- Custom colored blocks: supports specifying any color, instead of only 16 colors like concretes and wool blocks.
...
- Column building tool: quickly place blocks in a column.
...
- Colored tool: arbitrarily get and modify the color of blocks, only supports some blocks that supports custom coloring.
- Text copy tool: Copy and paste texts on the sign (including vanilla sign).
- Growth tool: Make creatures and crops grow or grow inversely, and can also make creatures freeze or unfreeze age when holding a golden dandelion on the offhand (26.1 version and above).
- Carrying tool: Pick a block or entity and carry to another place.
- Road tool: Quickly add or remove the line marks on the road.
- ……
