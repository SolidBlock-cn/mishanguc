...
- 修复文本预设中的默认文本大小为 8 和 4 而非 6 和 3 的问题。
- 修复在 `/mishanguc:signpreset list` 命令中，各项预设的文本提示中显示的是名称而非 ID 的问题。
- 修复在 26.1.2 版本的编辑告示牌界面中增减文本行时焦点异常的问题。
- 进一步优化了文本工具、颜色工具和搬运工具的实现方式，现在判断逻辑在客户端执行，执行失败时没有摇手动作且不会转交至服务器。服务器执行时依然会执行判断逻辑。
- 调整了生长工具对史莱姆、岩浆怪和硫方怪的处理方式，现在每次是大小加或减 1，而非翻倍或者减半，并且最大可设置为 8。对于硫方怪，只有当大小设置为 1 时，才会调整为幼年形态。
- 改善了数据标签工具、搬运工具、强制放置工具等物品在指向实体时对实体轮廓的渲染。现在当实体移动时，渲染的实体轮廓也会平滑地跟随移动。
- 修复使用搬运工具获取方块并覆盖已获取的实体时，消息提示的翻译参数错误的问题。
- 修复了使用搬运工具拾起并放置带有方块实体的方块时，没有加载方块实体数据或者加载的方块数据被无关的物品组件数据覆盖的问题。
- 修复了副手持有带有方块实体数据的方块时，柱建造工具未正确使用方块实体数据的问题。
    - 现在，在副手上持有带有方块实体数据的方块（例如有纹理的旗帜、装有物品的箱子、写着文本的告示牌、带有自定义染色且颜色确定的方块），或者持有已经装有方块的搬运工具时，主手使用强制放置工具、快速建造工具、柱建造工具都能正常使用副手上的方块的方块实体数据。
    - 注意：如果副手上的方块是自动决定颜色的自定义染色方块（例如从创造模式物品栏获取的），使用上述工具放置的方块可能不会正常着色。
    - 柱建造工具的效果可能会有细微变化。
- 现在在无权使用强制放置工具的情况下，尝试使用主手上的强制放置工具不会使用副手上的物品，也不会触发常规的方块破坏操作。
- 对于 26.2 版本：
    - 完整灯块的硫方怪原型为普通（`regular`）。
    - 道路方块的硫方怪原型为缓慢弹性（`slow_bouncy`）。
    - 自定义染色的方块（除了透明方块、半透明方块和不完整方块）的硫方怪原型与其基础方块一致。
    - 当自定义染色的方块被装进硫方怪时，其颜色与在物品栏中的形式一致。如果未确定颜色，则颜色会随时间改变，如果确定了颜色，则使用这个颜色。
- 部分更改参见 1.6.4 和 1.6.3 及相关 beta 版本的更新日志。

### 1.6.5-alpha.2

- 修复在 1.21.1 使用 NeoForge 和 Sinytra Connector 的情况下，持一些工具（如 ID 检查工具、数据标签工具、颜色工具）使用鼠标左键会意外地破坏方块的问题。
...
- 现在颜色工具的染色逻辑仅在服务器运行。
- 修复在 26.1 以上版本无法在服务器中启动的问题。

### 1.6.3-beta.4

- 修复模组中的 mixin 在混淆的版本中无法运行的问题。
- 改善了信标光柱的渲染，现在本模组的染色方块所导致的信标光柱颜色均为 alpha=255 的颜色。之前在光影下可能渲染不正常，现在应该正常了。
- 修复染色石头和染色下界传送门在物品栏内未被正确着色的问题。这一问题的原因是此模组未正确地使用 Minecraft 从 1.21.4 开始引入的物品模型映射。

### 1.6.3-beta.3

- 修复爆炸工具在使用时可能会导致使用副手物品的问题。

### 1.6.3-beta.2

- 修复文本复制工具在使用时可能会导致使用副手物品的问题。

### 1.6.3-beta.1

- 优化双持操作，在使用生长工具、搬运工具、瞬移工具和颜色工具时，即使操作失败，也不会使用副手物品。
- 现在在 26.1 中，副手持有金蒲公英时，对可冻龄生物使用生长工具可设置或解除冻龄（设置为幼年的同时会冻龄，设置为成年时会解除冻龄）。
- 修复在 26.1 之后的版本中，告示牌编辑屏幕会有背景的问题。
...
- 现在在告示牌编辑界面中输入特殊文本时，如果输入的内容无效，除了将文本框显示为红色之外，还会显示一个提示以表明错误。
- 修复了在输入告示牌预设名称的建议中，当名称包含特殊字符（例如符号、空格）时，会导致命令无效的问题。现在的建议会给告示牌名称加上引号。
- 改善告示牌编辑界面的操作，现在文本框的宽度会根据是否渲染滚动条自动决定。
- 修复当文本的 NBT 的 `textJson` 字段无效时，返回的文本组件的翻译键 `message.mishanguc.invalid_json` 不存在的问题。此外，该文本将显示为红色。
...
- 适用于 Minecraft 1.21.10 以上版本：调整文本渲染，以确保特殊的文本也能够被 Iris 等模组视为文本渲染，从而在启用光影时像普通文本一样拥有一些特殊属性。
- 适用于 Minecraft 1.21.10 以上版本：修复数据生成器存在无法运行的错误的问题。
- 调整了告示牌 `-texture` 的交互逻辑：现在在告示牌中，即使输入不存在的纹理 id，也会正常生效，渲染为黑紫方格且这过程中不会在日志中输出警告。
- 增加了特殊文本类型：`debug_text`，用法：在告示牌文本框中输入 `-debug_text 任意文本内容` 即可进行测试文本的渲染效果。请注意此情况下不会确保文本的渲染位置正确。
- 删除了一些冗余的代码。
- 修复游戏规则未及时同步的问题。
...
- 完善了告示牌文本中的 `-pattern`，现在的样式名称使用规范的名称，同时支持一些简略名称作为别称：
  ...
  - `u-turn-left-up`，别称：`u-turn-left-top`、`ulu`、`ult`
...
- 更新至 1.21.10 和 1.21.11。
- 修复一些情况下的告示牌编辑界面的文本没有渲染的问题。
- 优化告示牌编辑界面的操作逻辑以及文本框的显示效果。
- 修复在通过强制放置工具等物品放置的副手的方块或副手的搬运工具指定的方块时，方块实体数据未更新的问题。
- 修复搬运工具未正确处理方块实体的数据组件的问题。
- 修复告示牌编辑界面的文本描边控制按钮的操作逻辑异常的问题。
- 告示牌编辑界面中的文本阴影不再被视为不推荐开启。

### 1.5.3

- 对于 1.21.4 及以上版本，修复自定义染色的告示牌在物品栏中没有动态染色的问题。
- 其他更新请见 1.5.2、1.5.1 和 1.5.0 的更新日志。
...
- 优化 JSON 文本的解析方式。
- 现在在告示牌中输入特殊的文本时，如果其内容无效（例如 `-json` 和一段无效的 JSON 文本），文本框会显示为红色。
- 修复了特定情况下，带有直线和双斜线的道路方块侧面纹理缺失的问题。
- 修复告示牌编辑界面中，下划线按钮会错误修改文本的斜体的问题。

### 1.5.0

- 兼容 1.21.6。
- 修复在 1.21.5 中，一些灯块纹理错误的问题。
- 修复无法正确设置文本的 X 旋转的问题。
- 调整了一些内容的名称。
- 修复在告示牌中，输入“`-json`”开头的 JSON 文本后，再次打开编辑界面会缺失空格导致解析错误的问题。
...
- 修复 `mishanguc:includes_fluid` 物品组件被错误拼写为 `mishanguc:includes_field` 的问题。此前版本中如有物品被手动指定了物品组件，该组件将会失效。

### 1.4.6

- 修复一些自定义染色的告示牌在物品栏内的模型不正确的问题。

### 1.4.5

- 调整了墙上的灯方块和角落灯方块的放置规则。仅方块表面的中间位置有侧面形状或碰撞时，可放置灯，从而避免将灯放置在中间为空的侧面（例如墙上的灯块的侧面）。
- 修复了悬挂的告示牌杆的行为错误的问题。
- 修复了在告示牌编辑界面中，当编辑框过多导致部分编辑框没有显示时，这些未显示的编辑框仍会影响其他按钮的交互的问题。
- 加入了告示牌界面的简化模式的功能。按住 Shift 键点击“隐藏”按钮，即可开启或关闭简化模式。在简化模式下，即便有较多的文本，文本编辑界面也将在较小的范围内显示，在编辑文本的同时留下更多空间查看实际效果。按住 Shift 键，并将鼠标悬浮在“隐藏”按钮上，滚动鼠标滚轮，或者在“隐藏”按钮获得焦点时按住 Shift 键并按下向上/向下键，可调整文本编辑区域的高度。
...
- 现在在告示牌设置自定义的文本和描边颜色时，支持指定不透明度，并且支持以下十六进制格式：`#rgb`、`#rgba`、`#rrggbb`、`#rrggbbaa`，例如 `#f80` 和 `#ff8800` 表示橙色，`#0ff8` 和 `#00ffff88` 表示半透明的青色。
...
- 修复 `-pattern` 或 `-rect` 形式的文本在带有描边渲染时出现错误的问题。为与普通文本一致，有描边时不渲染阴影。
- 给告示牌的文本添加了 `outlineColorType` 字段，其值为 `auto`、`none` 或 `custom`，以修复当描边颜色正好为 `#fffffeff` 或 `#ffffffff` 时被识别为 none 或 auto 的问题。
...
- 修复无法正确设置文本的 X 旋转的问题。
- 调整了一些内容的名称。
- 修复 `#leaves` 标签因为含有在 1.19.2 中不存在的樱花而无效的问题。
- 修复此版本中的创建模式物品栏中的爆炸工具类型不正确的问题。
...
- 调整并修复了标签。
    ...
    - 将 `mishanguc:colored_light` 加入 `#mishanguc:lights` 方块标签。
...
- 调整了悬挂的告示牌以及告示牌杆的判定箱，使其略宽于碰撞箱，但窄于原先持有告示牌或告示牌杆时的判定箱。现在判定箱不会因持有告示牌或告示牌杆而变宽了。
