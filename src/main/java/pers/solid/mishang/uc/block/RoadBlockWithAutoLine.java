...
public class RoadBlockWithAutoLine extends AbstractRoadBlock implements RoadWithAutoLine {
        ...
        for (Map.Entry<Direction, RoadConnectionState> entry : connectionStateMap.entrySet()) {
          ...
          if (adjacentDirection == direction.getOpposite()) {
            ...
            if (connectionState.lineColor() == adjacentState.lineColor() || adjacentState.lineColor() == LineColor.UNKNOWN) {
                ...
                case YELLOW:
                  switch (type) {
                    case BEVEL:
                      switch (lineType) {
                        case DOUBLE:
                          road = ROAD_WITH_YELLOW_BA_DOUBLE_LINE;
                          break;
                        case THICK:
                          road = ROAD_WITH_YELLOW_BA_THICK_LINE;
                          break;
                        default:
                          road = ROAD_WITH_YELLOW_BA_LINE;
                          break;
                      }
                      break;
                    case RIGHT_ANGLE:
                      road = ROAD_WITH_YELLOW_RA_LINE;
                      break;
                    default:
                ...
                case UNKNOWN:
                  switch (type) {
                    case BEVEL:
                      switch (lineType) {
                        case DOUBLE:
                          road = ROAD_WITH_WHITE_BA_DOUBLE_LINE;
                          break;
                        case THICK:
                          road = ROAD_WITH_WHITE_BA_THICK_LINE;
                          break;
                        default:
                          road = ROAD_WITH_WHITE_BA_LINE;
                          break;
                      }
                      break;
                    case RIGHT_ANGLE:
                      road = ROAD_WITH_WHITE_RA_LINE;
                      break;
                    default:
                      throw new IllegalArgumentException();
                  }
              ...
              return road.getDefaultState().with(RoadWithAngleLine.FACING, HorizontalCornerDirection.fromDirections(direction, adjacentDirection));
            } else if (connectionState.lineColor() == LineColor.UNKNOWN) {
              continue;
            }
  ...
  private BlockState composeAngleLineWithOnePartOffset(LineColor lineColor, HorizontalCornerDirection facing, Direction.Axis axis, boolean isInwards) {
    ...
    switch (lineColor) {
      case YELLOW:
        block = (isInwards ? ROAD_WITH_YELLOW_RA_LINE_OFFSET_IN : ROAD_WITH_YELLOW_RA_LINE_OFFSET_OUT);
        break;
      default:
        block = (isInwards ? ROAD_WITH_WHITE_RA_LINE_OFFSET_IN : ROAD_WITH_WHITE_RA_LINE_OFFSET_OUT);
        break;
    }
  ...
  private BlockState composeAngleLineWithTwoPartsOffset(LineColor lineColor, HorizontalCornerDirection facing, boolean isInwards, RoadAutoLineType type) {
    ...
    if (type == RoadAutoLineType.RIGHT_ANGLE) {
      switch (lineColor) {
        case YELLOW:
          block = (isInwards ? ROAD_WITH_YELLOW_OFFSET_IN_RA_LINE : ROAD_WITH_YELLOW_OFFSET_OUT_RA_LINE);
          break;
        default:
          block = (isInwards ? ROAD_WITH_WHITE_OFFSET_IN_RA_LINE : ROAD_WITH_WHITE_OFFSET_OUT_RA_LINE);
          break;
      }
    } else {
      switch (lineColor) {
        case YELLOW:
          block = (isInwards ? ROAD_WITH_YELLOW_OFFSET_IN_BA_LINE : ROAD_WITH_YELLOW_OFFSET_OUT_BA_LINE);
          break;
        default:
          block = (isInwards ? ROAD_WITH_WHITE_OFFSET_IN_BA_LINE : ROAD_WITH_WHITE_OFFSET_OUT_BA_LINE);
          break;
      }
    }
  ...
  private static BlockState composeOffsetStraightLine(Direction offsetDirection, int offsetLevel, LineColor color) {
    ...
    switch (offsetLevel) {
      case 114514:
        return ROAD_WITH_WHITE_YELLOW_DOUBLE_LINE.getDefaultState().with(Properties.HORIZONTAL_FACING, offsetDirection.getOpposite());
      case 2:
        switch (color) {
          case YELLOW:
            block = ROAD_WITH_YELLOW_OFFSET_LINE;
            break;
          default:
            block = ROAD_WITH_WHITE_OFFSET_LINE;
            break;
        }
        return block.getDefaultState().with(Properties.HORIZONTAL_FACING, offsetDirection.getOpposite());
      case 1:
        switch (color) {
          case YELLOW:
            block = ROAD_WITH_YELLOW_HALF_DOUBLE_LINE;
            break;
          default:
            block = ROAD_WITH_WHITE_HALF_DOUBLE_LINE;
            break;
        }
        return block.getDefaultState().with(Properties.HORIZONTAL_FACING, offsetDirection.getOpposite());
      default:
        ...
        return block.getDefaultState().with(Properties.AXIS, offsetDirection.rotateYClockwise().getAxis());
    }
  }
