...
public class HungSignBlock extends Block implements Waterloggable, BlockEntityProvider, BlockResourceGenerator {
  ...
  public ActionResult onUse(
    ...
    if (!state.get(AXIS).test(hit.getSide())) {
      ...
      for (BlockEntity value : worldChunk.getBlockEntities().values()) {
        if (value instanceof HungSignBlockEntity) {
          HungSignBlockEntity hungSignBlockEntity = (HungSignBlockEntity) value;
  ...
  public RecipeJsonFactory getCraftingRecipe() {
    ...
    return ShapedRecipeJsonFactory.create(this, 6)
        ...
        .criterionFromItem("has_base_block", WallSignBlocks.INVISIBLE_WALL_SIGN)::offerTo;
  }
