...
public class DataTagToolItem extends BlockToolItemWithEntity implements InteractsWithEntity, RendersBeforeOutline {
  ...
  public static class BlockDataReceiver implements ClientPlayNetworking.PlayPayloadHandler<GetBlockDataPayload> {
    ...
    public void receive(GetBlockDataPayload payload, ClientPlayNetworking.Context context) {
      ...
      if (hasData) {
        ...
        client.execute(() -> {
          client.inGameHud.getChatHud().addMessage(TextBridge.translatable("debug.mishanguc.dataTag.block.header", String.format("%s %s %s", blockPos.getX(), blockPos.getY(), blockPos.getZ()), block.getName().formatted(Formatting.BOLD)).formatted(Formatting.YELLOW));
          client.inGameHud.getChatHud().addMessage(NbtPrettyPrinter.serialize(blockData));
        });
      } else {
        // 此时认为该方块没有数据。
        client.execute(() -> client.inGameHud.getChatHud().addMessage(TextBridge.translatable("debug.mishanguc.dataTag.block.null", String.format("%s %s %s", blockPos.getX(), blockPos.getY(), blockPos.getZ()), block.getName().formatted(Formatting.BOLD)).formatted(Formatting.RED)));
      }
    }
  }
  ...
  public static class EntityDataReceiver implements ClientPlayNetworking.PlayPayloadHandler<GetEntityDataPayload> {
    ...
    public void receive(GetEntityDataPayload payload, ClientPlayNetworking.Context context) {
      ...
      final MinecraftClient client = context.client();
      client.inGameHud.getChatHud().addMessage(TextBridge.translatable("debug.mishanguc.dataTag.entity.entity", String.format("%s %s %s", entityPos.getX(), entityPos.getY(), entityPos.getZ()), TextBridge.literal("").append(entityName).formatted(Formatting.BOLD)).formatted(Formatting.YELLOW));
