...
public class CarryingToolItem extends BlockToolItem
  ...
  private static MutableText getEntityName(@NotNull ItemStack stack) {
    ...
    if (nbt.contains("holdingEntityName", NbtType.STRING)) {
      ...
      return Registry.ENTITY_TYPE.getOrEmpty(holdingEntityType).map(entityType -> entityType.getName().shallowCopy()).orElseGet(() -> TextBridge.literal(String.valueOf(holdingEntityType)));
    } else {
  ...
  public @NotNull ModelJsonBuilder getItemModel() {
    return ModelJsonBuilder.create(Models.HANDHELD).setTextures(Texture.layer0(getTextureId()))  // TODO: 2023/4/15, 015 check
