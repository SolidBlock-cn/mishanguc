...
public class RoadConnectionStateDebuggingToolItem extends BlockToolItem {
     ...
     * 向聊天框广播各个方向的道路连接状态。
     */
    public static ActionResult sendMessageOfState(PlayerEntity playerEntity, BlockState blockState, BlockPos blockPos) {
        Block block = blockState.getBlock();
        if (!(block instanceof Road)) {
            playerEntity.sendMessage(new TranslatableText("debug.mishanguc.notRoad"), false);
    ...
    public ActionResult useOnBlock(PlayerEntity player, World world, BlockHitResult blockHitResult, Hand hand, boolean fluidIncluded) {
        return sendMessageOfState(player,world.getBlockState(blockHitResult.getBlockPos()),blockHitResult.getBlockPos());
    }
