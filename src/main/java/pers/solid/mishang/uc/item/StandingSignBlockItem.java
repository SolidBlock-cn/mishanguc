...
public class StandingSignBlockItem extends NamedBlockItem {
  ...
  public void appendTooltip(
    ...
    if (nbt == null) return;
    final List<TextContext> frontTexts = getTextContextsFromNbt(nbt.get("frontTexts")).collect(Collectors.toList());
    ...
    final List<TextContext> backTexts = getTextContextsFromNbt(nbt.get("backTexts")).collect(Collectors.toList());
  ...
  public Text getName(ItemStack stack) {
    ...
    final List<MutableText> texts = Stream.concat(getTextContextsFromNbt(nbt.get("frontTexts")), getTextContextsFromNbt(nbt.get("backTexts"))).map(TextContext::asStyledText).limit(20).collect(Collectors.toList());
    if (!texts.isEmpty()) {
