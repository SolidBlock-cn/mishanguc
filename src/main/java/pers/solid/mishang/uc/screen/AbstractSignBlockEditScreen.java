...
public abstract class AbstractSignBlockEditScreen<T extends BlockEntityWithText> extends Screen {
  ...
  public @NotNull TextFieldListWidget textFieldListWidget = new TextFieldListWidget(this, MinecraftClient.getInstance(), width, height, 25, height - 65, 18);
  ...
  public final SignPresetGridWidget signPresets = SignPresetGridWidget.createAllWidgets(this, MinecraftClient.getInstance(), height, 72, height - 68);
  ...
  public final FloatButtonWidget outlineColorButton = new FloatButtonWidget(0, 0, 70, 20, TextBridge.translatable("message.mishanguc.outline_color"), button -> {
    ...
    } else if (colorId == -3 && textFieldListWidget.getSelectedOrNull() != null) {
      return MishangUtils.describeColor(textFieldListWidget.getSelectedOrNull().textContext.outlineColor);
    } else if (colorId > 15 && textFieldListWidget.getSelectedOrNull() != null) {
    ...
    if (textFieldListWidget.isSimplified()) {
      ...
      .setTooltipSupplier(value -> {
        if (!hidden && hasShiftDown() || textFieldListWidget.isSimplified()) {
          return Text.empty()
              .append(Text.translatable("message.mishanguc.simplify.height", textFieldListWidget.cuttingHeight).styled(style -> style.withColor(0xffd0d0d0)))
  ...
  public AbstractSignBlockEditScreen(T entity, BlockPos blockPos, @UnmodifiableView List<TextContext> initialTexts) {
  ...
  protected void init() {
    super.init();
    textFieldListWidget.updateSize(width, height, 25, height - 65);
    ...
    if (initialTexts != null) {
      ...
      if (!textFieldListWidget.children().isEmpty()) {
        textFieldListWidget.setFocused(textFieldListWidget.children().get(textFieldListWidget.children().size() - 1));
      }
      initialTexts = null;
    }
  ...
  protected void initTextHolders() {
    ...
    if (textHolders.size() == 1) {
      textHolders.get(0).setWidth(200);
      textHolders.get(0).setPosition(width / 2 - 100, 35);
    } else if (textHolders.size() >= 2) {
      textHolders.get(0).setWidth(160);
      textHolders.get(0).setPosition(width / 2 - 160, 35);
      textHolders.get(1).setWidth(160);
      textHolders.get(1).setPosition(width / 2, 35);
    }
  }

  protected void initSignPresets() {
    addDrawableChild(signPresets);
    signPresets.updateSize(width, height, 72, height - 68);
    signPresets.setRenderHorizontalShadows(false);
    signPresets.setRenderSelection(false);
    signPresets.setScrollAmount(signPresets.getScrollAmount());
    signPresets.setRenderBackground(false);
  }
  ...
  public boolean mouseClicked(double mouseX, double mouseY, int button) {
    ...
    for (Element element : this.children()) {
      ...
      if (element instanceof SignPresetGridWidget || !textFieldListWidget.children().isEmpty()) {
        setFocused(textFieldListWidget);
      } else if (element instanceof EntryListWidget<?> || element instanceof TextFieldWidget) {
  ...
  private void customValueStartAccepting(FloatButtonWidget floatButtonWidget) {
    ...
    if (floatButtonWidget == colorButton) {
      ...
      customValueTextField.setChangedListener(s -> {
        ...
        final Integer parse = MishangUtils.parseColor(text);
        if (parse == null) {
  ...
  public void removed() {
    ...
    for (TextFieldListWidget.Entry entry : textFieldListWidget.children()) {
      list.add(entry.textContext.createNbt());
    }
  ...
  protected void clearAndInit() {
    final double scrollAmountBeforeClear = textFieldListWidget.getScrollAmount();
