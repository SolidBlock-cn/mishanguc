...
public class TextFieldListWidget extends AlwaysSelectedEntryListWidget<TextFieldListWidget.Entry> {
  ...
  public void setWidth(int width) {
    ...
    final boolean scrollbarVisible = isScrollbarVisible();
  ...
  private @NotNull Entry createEntry(@NotNull TextContext textContext) {
    final TextFieldWidget textFieldWidget = new TextFieldWidget(Objects.requireNonNull(MinecraftClient.getInstance().textRenderer, "textRenderer"), 2, 0, signBlockEditScreen.width - 4, 15, TextBridge.empty());
    ...
    if (textContext.extra != null) {
      ...
      if (textContext.text.getContent() instanceof PlainTextContent plainTextContent && textContext.text.getSiblings().isEmpty() && textContext.text.getStyle().isEmpty()) {
        ...
        textFieldWidget.setText("-json " + Text.Serialization.toJsonString(textContext.text, signBlockEditScreen.registryLookup));
      }
    }
