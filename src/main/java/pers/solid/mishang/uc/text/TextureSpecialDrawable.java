...
public record TextureSpecialDrawable(@NotNull Identifier identifier, @NotNull TextContext textContext) implements SpecialDrawable {
  ...
  /**
   * 说明：原版的文本的实现见于 {@link net.minecraft.client.texture.SpriteAtlasGlyphs.AtlasGlyph#draw(Matrix4f, VertexConsumer, int, float, float, float, int)}，此处的逻辑略有不同。
   */
  ...
  public void drawInternal(Matrix4f matricesEntry, VertexConsumerProvider.Immediate vertexConsumers, int light, float x, float y) {
    ...
    if (resource.isEmpty()) {
      layer = RenderLayers.text(MissingSprite.getMissingSpriteId());
    } else {
      layer = RenderLayers.text(identifier);
    }
