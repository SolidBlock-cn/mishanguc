...
/**
 * 实现此方法的物品，在玩家持有时（无论是主手还是副手），均会调用 {@link #renderBeforeOutline(WorldRenderContext, HitResult, Hand)} 方法。这是在 {@link pers.solid.mishang.uc.MishangucClient} 中注册的。<p>
 * <p>
...
public interface RendersBeforeOutline {
  /**
   * 玩家持有此物品时，客户端进行的渲染操作。仅限客户端执行。覆盖此方法时，请一并注解上 {@link net.fabricmc.api.Environment}。
   *
  ...
  WorldRenderEvents.BeforeBlockOutline RENDERER = (context, hitResult) -> {
    ...
    for (final Hand hand : new Hand[]{Hand.MAIN_HAND, Hand.OFF_HAND}) {
      ...
      if (stackInHand.getItem() instanceof RendersBeforeOutline rendersBeforeOutline) {
        rendersBeforeOutline.renderBeforeOutline(context, hitResult, hand);
      }
    }
    return true;
  };
