...
public class ARRPMain implements ModInitializer {
  ...
  private static void addGlassHandrailsRecipes() {
    ...
    HandrailBlocks.DECORATED_IRON_HANDRAILS.forEach((dyeColor, glassHandrailBlock) -> {
      final TagKey<Item> dyeKey = TagKey.of(RegistryKeys.ITEM, new Identifier("c", dyeColor.asString() + "_dyes"));
  ...
  private static void addRecipeForGlassHandrail(GlassHandrailBlock output, ItemConvertible frame, ItemConvertible decoration, ItemConvertible base, int outputCount, @Nullable String group) {
    final ShapedRecipeJsonBuilder recipe = ShapedRecipeJsonBuilder.create(output.getRecipeCategory(), output, outputCount)
        .pattern("XXX")
        .pattern("oMo")
        .pattern("nnn")
        .input('X', frame)
  ...
  private static void addRoadPalingRecipes() {
    ...
    PACK.addRecipeAndAdvancement(RoadBlocks.ROAD_BLOCK.getRecipeId().brrp_suffixed("_from_paling"), SingleItemRecipeJsonBuilder.createStonecutting(Ingredient.fromTag(roadBlocks), RoadBlocks.ROAD_BLOCK.getRecipeCategory(), RoadBlocks.ROAD_BLOCK)
    ...
    PACK.addRecipeAndAdvancement(roadSlabBlock.getRecipeId().brrp_suffixed("_from_paling"), SingleItemRecipeJsonBuilder.createStonecutting(Ingredient.fromTag(roadSlabs), roadSlabBlock.getRecipeCategory(), roadSlabBlock)
