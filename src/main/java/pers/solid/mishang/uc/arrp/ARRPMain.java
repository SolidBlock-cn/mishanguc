...
public class ARRPMain implements RRPPreGenEntrypoint {
  ...
  private static @Nullable JFace face(@Nullable String texture, @Nullable Direction direction) {
    ...
    if (direction != null) face.cullface(direction);
    return face;
  }

  private static JFaces faces(@Nullable String down, @Nullable String up, @Nullable String north,
                              @Nullable String south, @Nullable String west, @Nullable String east) {
    JFaces faces = new JFaces();
    faces.down(face(down, Direction.DOWN)).up(face(up, Direction.UP)).north(face(north, Direction.NORTH)).south(face(south, Direction.SOUTH)).west(face(west, Direction.WEST)).east(face(east, Direction.EAST));
    return faces;
  }

  private static JFaces faces(Map<Direction, @Nullable String> map) {
    return faces(map.get(Direction.DOWN), map.get(Direction.UP), map.get(Direction.NORTH),
            map.get(Direction.SOUTH), map.get(Direction.WEST), map.get(Direction.EAST));
  }
    ...
    PACK.addModel(
            JModel.model("block/cube")
                    .element(
                            ...
                            fullElement(faces("line_bottom",
                                    "line_top", "line_side", null, null, "line_side"))
                    ).textures(
                            JModel.textures().particle("#base").var("line_bottom", "#line_top")
                    ),
  ...
  private static void addBlockLootTable(RuntimeResourcePack PACK, String name) {
    PACK.addLootTable(new Identifier("mishanguc", "blocks/" + name),
  ...
  private static void addCubeAll(RuntimeResourcePack PACK, String path, String all) {
    PACK.addModel(JModel.model("block/cube_all").textures(JModel.textures().var("all", blockString(all))),
            blockIdentifier(path));
  }

  private static void addSlabAll(RuntimeResourcePack PACK, String path, String all) {
    PACK.addModel(JModel.model("block/slab").textures(JModel.textures()
            .var("top", blockString(all))
            .var("side", blockString(all))
            .var("bottom", blockString(all))), blockIdentifier(path));
    PACK.addModel(JModel.model("block/slab_top").textures(JModel.textures()
            .var("top", blockString(all))
            .var("side", blockString(all))
            .var("bottom", blockString(all))), blockIdentifier(path + "_top"));
  }
