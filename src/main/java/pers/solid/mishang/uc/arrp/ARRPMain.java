...
public class ARRPMain implements ModInitializer {
  ...
  private static void addGlassHandrailsRecipes() {
    ...
    HandrailBlocks.DECORATED_IRON_HANDRAILS.forEach((dyeColor, glassHandrailBlock) -> {
      final TagKey<Item> dyeKey = TagKey.of(Registry.ITEM_KEY, new Identifier("c", dyeColor.asString() + "_dyes"));
      PACK.addRecipeAndAdvancement(glassHandrailBlock.getRecipeId(), ShapedRecipeJsonBuilder.create(glassHandrailBlock, 4)
          .pattern("XXX")
          .pattern("oMo")
          .pattern("nnn")
    ...
    for (GlassHandrailBlock output : List.of(
      ...
      final Identifier woodId = Registry.ITEM.getId(wood);
      final Item planks = Registry.ITEM.get(new Identifier(woodId.getNamespace(), woodId.getPath().replace("wood", "planks").replace("hyphae", "planks")));
  ...
  private static void addRecipeForGlassHandrail(GlassHandrailBlock output, ItemConvertible frame, ItemConvertible decoration, ItemConvertible base, int outputCount, @Nullable String group) {
    final ShapedRecipeJsonBuilder recipe = ShapedRecipeJsonBuilder.create(output, outputCount)
        .pattern("XXX")
        .pattern("oMo")
        .pattern("nnn")
        .input('X', frame)
    ...
    if (base != frame) {
      // 该版本中不允许重复的版本名，所以需要特殊处理。
      recipe.criterionFromItem(base);
    }
    PACK.addRecipeAndAdvancement(output.getRecipeId(), recipe
        .setCustomRecipeCategory("handrails")
        .group(group));
  }
  ...
  private static void addRecipeForGlassHandrail(GlassHandrailBlock output, TagKey<Item> frame, String frameCriterionName, ItemConvertible decoration, ItemConvertible base, int outputCount) {
    PACK.addRecipeAndAdvancement(output.getRecipeId(), ShapedRecipeJsonBuilder.create(output, outputCount)
        .pattern("XXX")
        .pattern("oMo")
        .pattern("nnn")
        .input('X', frame)
  ...
  private static void addRecipesForInvisibleSigns() {
    ...
    PACK.addRecipeAndAdvancement(BRRPUtils.getRecipeId(WallSignBlocks.INVISIBLE_WALL_SIGN),
        ShapedRecipeJsonBuilder.create(WallSignBlocks.INVISIBLE_WALL_SIGN, 9)
            .pattern(".#.")
            .pattern("#o#")
            .pattern(".#.")
    ...
    PACK.addRecipeAndAdvancement(BRRPUtils.getRecipeId(WallSignBlocks.INVISIBLE_GLOWING_WALL_SIGN),
        ShapedRecipeJsonBuilder.create(WallSignBlocks.INVISIBLE_GLOWING_WALL_SIGN, 3)
            .pattern("---")
            .pattern("###")
  ...
  private static void addRoadPalingRecipes() {
    // 将带有标线的道路重置为不带标线的道路。
    final TagKey<Item> roadBlocks = TagKey.of(Registry.ITEM_KEY, new Identifier("mishanguc", "road_blocks"));
    PACK.addRecipeAndAdvancement(RoadBlocks.ROAD_BLOCK.getRecipeId().brrp_suffixed("_from_paling"), SingleItemRecipeJsonBuilder.createStonecutting(Ingredient.fromTag(roadBlocks), RoadBlocks.ROAD_BLOCK)
        .criterionFromItemTag("has_road_block", roadBlocks)
        .setCustomRecipeCategory("roads"));
    final TagKey<Item> roadSlabs = TagKey.of(Registry.ITEM_KEY, new Identifier("mishanguc", "road_slabs"));
    final AbstractRoadSlabBlock roadSlabBlock = RoadSlabBlocks.BLOCK_TO_SLABS.get(RoadBlocks.ROAD_BLOCK);
    PACK.addRecipeAndAdvancement(roadSlabBlock.getRecipeId().brrp_suffixed("_from_paling"), SingleItemRecipeJsonBuilder.createStonecutting(Ingredient.fromTag(roadSlabs), roadSlabBlock)
