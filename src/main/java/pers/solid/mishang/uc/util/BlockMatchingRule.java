...
public abstract class BlockMatchingRule implements StringIdentifiable {
   ...
   * @return 包含这些坐标的链式集合。
   */
  public Set<BlockPos> getPlainValidBlockPoss(@NotNull World world, @NotNull BlockPos centerPos, @NotNull Direction side, int range) {
    final LinkedHashSet<BlockPos> set = Sets.newLinkedHashSet();
    final Direction.Axis axis = side.getAxis();
    for (BlockPos pos : BlockPos.iterateOutwards(centerPos, axis == Direction.Axis.X ? 0 : range, axis == Direction.Axis.Y ? 0 : range, axis == Direction.Axis.Z ? 0 : range)) {
      final BlockPos offsetPos = pos.offset(side);
      final boolean isValid = world.getBlockState(offsetPos).getCollisionShape(world, pos).getFace(side.getOpposite()).isEmpty();
      if (!isValid || !this.match(world.getBlockState(pos), world.getBlockState(centerPos))) continue;
      if (pos.equals(centerPos)) {
        set.add(pos.toImmutable());
        continue;
      }
      for (BlockPos pos1 : BlockPos.iterateOutwards(pos, axis == Direction.Axis.X ? 0 : 1, axis == Direction.Axis.Y ? 0 : 1, axis == Direction.Axis.Z ? 0 : 1)) {
        if (set.contains(pos1.toImmutable())) set.add(pos.toImmutable());
      }
    }
    return set;
  }
